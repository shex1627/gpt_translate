...
def get_article_summary(all_context_text: str, question: str, language: str = "Chinese"):
    ...
    responses = parallel_send_chatcomplete_api(trunk_messages, gpt4_config)
...
def agggregate_article_summary(article_summaries: str, question: str, language: str = "Chinese"):
    aggregate_answers_msg = aggregate_article_prompt(question, article_summaries, 
                                                     ...
                                                     language=language)
    response = openai.ChatCompletion.create(**gpt4_config, messages=aggregate_answers_msg)
    return response
    sentences = []
    sentences.append("=====================================\n")
    sentences(f"question: {question}\n")
    sentences(f"final summarys\n")
    sentences("=====================================\n")
    for sentence in response.choices[0]['text'].split(st.session_state.period):
        if sentence:
...
question = st.text_input("question prompt", max_chars=30)
...
if st.session_state.get("results", pd.DataFrame()).shape[0] > 0:
    ...
    with open(OUTPUT_FILE_PATH, "w") as f:
        output["-".join([question, st.session_state.language, str(st.session_state.seed)])] = {"article_summary_sentences": trunk_processed, 
